                ...
                <div class="flex items-start justify-start flex-shrink-0">
                    <div class="flex items-center justify-center w-full pb-6 space-x-4 md:justify-start">
                        <div class="flex flex-col items-start justify-start space-y-2">
                            ...
                            <p class="text-sm leading-4 text-gray-600 dark:text-gray-400">{{$order->address->city}}
                                , {{$order->address->state}}, {{$order->address->zip_code}}</p>
                            <p class="text-sm leading-4 cursor-pointer dark:text-gray-400">{{__('form.phone')}}
            ...
            <div class="flex flex-wrap items-center pb-4 mb-10 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="w-full px-4 mb-4 md:w-1/4">
                    <p class="mb-2 text-sm font-medium leading-5 text-gray-800 dark:text-gray-400 ">
                        {{__('form.currency_price')}}: </p>
                    <p class="text-base font-semibold leading-4 text-gray-800 dark:text-gray-400">
                        {{Number::Currency(1, 'USD')}}
                        = {{Number::Currency($order->currency_price, $order->currency)}}</p>
                </div>
                <div class="w-full px-4 mb-4 md:w-1/4">
                    <p class="mb-2 text-sm font-medium leading-5 text-gray-800 dark:text-gray-400 ">
                        {{__('front.total')}}: </p>
                    <p class="text-base font-semibold leading-4 text-blue-600 dark:text-gray-400">
                        {{Number::Currency($order->grand_total * $order->currency_price, $order->currency)}}</p>
                </div>
